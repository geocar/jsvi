/* 
    ...
    Copyright (C) 2006-2008 Geo Carncross (Internet Connection, Inc.)
...
/* TODO
   ...
   msie doubleclick needs second on cursor (why?)


sortof bugs:
   fix styling where cursor is (hover are)

   the toolbar is kind of ugly- possibly replace with an image-version-
   or more likely- replace with styled buttons

ideas:
   uneditable sections? (think CMS)
...
function _suggest(e) {
	...
	da.style.display = 'block';
	da.style.backgroundColor = palette[0];
	da.style.backgroundImage = 'url(redline.png)';
	da.style.backgroundPosition = 'bottom';
	da.style.backgroundRepeat = 'repeat-x';
...
function _cursortoxy(x,y) {
	...
	var sx = cursorx;
	cursorx = parseInt(x / (cursor.offsetWidth+1));
	term_redraw();
...
function term_command(s) {
	...
	if (cmd2 == 'wq') {
	...
	} else if (s.substr(i,4) == 'help' || s.substr(i,5) == 'about') {
		...
		for (y = 0; y < file.length; y++) {
			base = 0;
			cursory = y;
			term_command(s.substr(i+1, (s.length-i)-1));
		}
...
function term_calcx() {
	if (cursorx != cursor._lastx) {
		cursor.style.left = (cursorx * (cursor.offsetWidth+1)) + 'px';
...
function _redraw_term() {
	...
	for (y = 0; y < h; y++) {
		...
		if (y == (h-1)) {
			...
			for (j = 0; j < vj.length; j++) {
				...
				var vm = _word(vx);
				if (j != 0) p++;
				if (brokenwords[vm] && !safewords[vm]) {
					g = g.substr(0, p)
		...
		if (term.childNodes.length > y) {
			...
			zx.style.marginBottom='4px'; // height of redline.png
		}
		...
		for (j = 0; j < x.length; j++) {
			...
			if (gx != cx) {
				...
				if (gx > 255) {
					...
					ax.style.backgroundImage = 'url(redline.png)';
					ax.style.backgroundPosition = 'bottom';
					ax.style.backgroundRepeat = 'repeat-x';
	...
	if (!spelling && tospell > 0 && false) {//xxx spellcheck disabled
		spelling = true;
		var xh = _xhttp();
...
function term_resize() {
	...
	h = term.offsetWidth;
	r = (cursor.offsetWidth+1); // 1 px padding
	nh = (h/r);
...
function editor(t) {
	...
	_cbd('onselect', _cancel_ev);
	_cbd('onselectstart', _cancel_ev);
